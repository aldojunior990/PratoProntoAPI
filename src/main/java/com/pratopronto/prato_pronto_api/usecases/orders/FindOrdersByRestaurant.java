...
@Service
public class FindOrdersByRestaurant implements UseCaseContract<String, ResponseEntity<List<FindOrderDTO>>> {
    ...
    public ResponseEntity<List<FindOrderDTO>> execute(String id) {
        try {
            ...
            List<Order> orders = orderGateway.findAllByRestaurantID(id);
