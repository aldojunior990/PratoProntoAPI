...
@Service
public class UpdateCreditCard implements UseCaseContract<HttpRequestDTO<CreditCardDTO>, ResponseEntity<String>> {
    ...
    public ResponseEntity<String> execute(HttpRequestDTO<CreditCardDTO> data) {
        try {
            ...
            boolean isSaved = creditCardGateway.update(CreditCard.with(
                    data.content().id(),
                    data.content().token(),
                    data.content().holder(),
                    data.content().flag(),
            ...
            return ResponseEntity.ok("Cartão de credito atualizado com sucesso");
